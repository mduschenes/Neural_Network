...
def run_rep(data,rep='tsne',M=2,perp=30.0):
	n,N = np.shape(data)
	tol = 1e-6
...
if __name__ == "__main__":
	...
	data_params =  {'data_files': data_files,
					...
					'data_dir': 'dataset/tsne/dataset_May21_12pm/',
					'one_hot': [False]}
	...
	for t in sorted(data_types_config):
		...
		for r in data_reps:
			...
			if 'gauge' in t or r == 'pca':
				continue
		
			# Check if Data Exists
			params = data_params.copy()
			file_name = r+'_'+t+'NONE.npz'
			params['data_files'] = file_name
			data = Data_Proc.importer(params,data_obj_format='dict',format='npz')
			if data is not None:
				print('Previous Data Found',file_name)
				Y[r][t] = data[0][file_name.split('.')[0]].item()
				print('keys',Y[r][t].keys())
			else:
				print('New Data',file_name)
				
				for k in data_keys[t]:   
					print(r,t,k)
					Y[r][t][k] = run_rep(data_typed[t][k],rep=r)
					
					
					 pylab.scatter(Y[:, 0], Y[:, 1], 20, np.reshape(data_typed[(
							 'temperatures'+'_'+t)][k],(-1,)))
